...
import {
  parseSpacingToMeters,
  realWorldSizeToPixels,
  calculateIconSize,
  formatRealWorldSize,
} from '@/utils/plantSizes'
import {
  getPlantBorderColor,
  getShapeColor,
  hexToRgba,
...
const CanvasElementComponent = ({
  ...
  if (element.type === 'plant') {
    ...
    const clickableSize = {
      ...
      height: Math.max(
        minClickableSize,
        Math.min(maxClickableSize, pixelSize.height * scaleFactor)
      ),
    }
      ...
      >
        ...
        {showTooltip && (
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm rounded-lg shadow-lg border whitespace-nowrap z-20 min-w-max">
            <div className="font-semibold" style={{ color: borderColor }}>
              {element.plant?.name}
            </div>
            <div className="text-gray-600 dark:text-gray-400 text-xs">
              Espaçamento: {element.plant?.spacing}
            </div>
            <div className="text-gray-600 dark:text-gray-400 text-xs">
              Área: {formatRealWorldSize(realWorldSize)}
            </div>
  ...
  if (element.type === 'terrain') {
    ...
    if (brushType === 'path' && element.pathPoints && element.pathPoints.length > 1) {
      ...
      return (
        <motion.div
          ...
          onMouseLeave={handleMouseLeave}
        >
          <svg width={maxX - minX + 10} height={maxY - minY + 10} className="absolute inset-0">
            {/* Path shadow */}
            <path
              d={pathD}
          ...
          {showTooltip && (
            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm rounded-lg shadow-lg border whitespace-nowrap z-20 min-w-max">
              ...
              <div className="text-gray-600 dark:text-gray-400 text-xs">
                Comprimento: ~
                {Math.round(Math.sqrt(Math.pow(maxX - minX, 2) + Math.pow(maxY - minY, 2)) / 10)}m
              </div>
            </div>
          )}
        </motion.div>
    ...
    const clickableWidth = Math.max(
      minClickableSize,
      Math.min(maxClickableSize, terrainWidth * scaleFactor)
    )
    const clickableHeight = Math.max(
      minClickableSize,
      Math.min(maxClickableSize, terrainHeight * scaleFactor)
    )
    ...
    return (
      <motion.div
        ...
        style={{
          ...
          backgroundImage: getTerrainPattern(
            element.texture || 'default',
            element.terrain?.color || '#666'
          ),
      ...
      >
        ...
        {showTooltip && (
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm rounded-lg shadow-lg border whitespace-nowrap z-20">
            ...
            <div className="text-gray-600 dark:text-gray-400 text-xs">
              Tamanho: {Math.round((element.width || 0) / 10)}×
              {Math.round((element.height || 0) / 10)}m
            </div>
            <div className="text-gray-600 dark:text-gray-400 text-xs">
